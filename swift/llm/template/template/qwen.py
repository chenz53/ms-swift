...
register_template(
    QwenTemplateMeta(LLMTemplateType.qwq_preview, default_system=qwq_preview_system)
)
...
register_template(
    QwenTemplateMeta(LLMTemplateType.qwen3_nothinking, default_system=None)
)

register_template(
    QwenTemplateMeta(
        LLMTemplateType.qwen3_coder, default_system=None, agent_template="qwen3_coder"
    )
)
...
class QwenPRMTemplate(Template):
    ...
    def _preprocess_inputs(
        ...
        total_content = "\n".join(
            [message["content"] or "" for message in inputs.messages]
        )
        if self.cot_process_placeholder not in total_content:
            inputs.messages[-1]["content"] = (
                inputs.messages[-1]["content"] + self.cot_process_placeholder
            )
...
register_template(
    Qwen2_5MathTemplateMeta(
        LLMTemplateType.qwen2_5_math_prm, template_cls=QwenPRMTemplate
    )
)
...
class QwenVLTemplate(Template):
    ...
    def _load_image(image, load_images: bool):
        if (
            ...
            and (image.startswith("data:") or len(image) > 200)
        ):
            load_images = True
    ...
    def replace_ref(
        self, ref: str, index: int, inputs: StdTemplateInputs
    ) -> List[Context]:
        return [f"<ref>{ref}</ref>"]

    def replace_bbox(
        self, bbox: List[int], index: int, inputs: StdTemplateInputs
    ) -> List[Context]:
        return [f"<box>{self._get_bbox_str(bbox)}</box>"]


register_template(
    QwenTemplateMeta(MLLMTemplateType.qwen_vl, template_cls=QwenVLTemplate)
)
...
register_template(
    QwenTemplateMeta(MLLMTemplateType.qwen_audio, template_cls=QwenAudioTemplate)
)
...
register_template(
    QwenTemplateMeta(MLLMTemplateType.qwen2_audio, template_cls=Qwen2AudioTemplate)
)
...
class Qwen2VLTemplate(Template):
    ...
    def replace_tag(
        ...
        if media_type == "image":
            inputs.images[index] = fetch_image(
                {"image": inputs.images[index]}, **kwargs
            )
            if self.mode == "lmdeploy":
            ...
            if self.version == "v2_5":
                ...
                if self.mode == "vllm":
                    ...
                    inputs.mm_processor_kwargs.setdefault("video_metadata", []).append(
                        video_metadata
                    )
                    tokens = ["<|video_pad|>"]
    ...
    def forward_context(self, model, inputs):
        if not self.padding_free or self.transformers_version >= version.parse(
            "4.53.0.dev"
        ):
        ...
        if self.version == "v2":
            from transformers.models.qwen2_vl import (
                modeling_qwen2_vl as modeling_module,
            )
        elif self.version == "v2_5":
            from transformers.models.qwen2_5_vl import (
                modeling_qwen2_5_vl as modeling_module,
            )
        elif self.version == "omni_v2_5":
            from transformers.models.qwen2_5_omni import (
                modeling_qwen2_5_omni as modeling_module,
            )
...
register_template(
    QwenTemplateMeta(MLLMTemplateType.qwen2_vl, template_cls=Qwen2VLTemplate)
)
...
register_template(
    QwenTemplateMeta(MLLMTemplateType.qwen2_5_vl, template_cls=Qwen2_5VLTemplate)
)
        ...
        for media_type in ["images", "videos"]:
            ...
            if mm_data:
                if media_type == "images":
                    ...
                    splited_tokens = self._split_list(
                        media_inputs["input_ids"][0].tolist(), split_token
                    )
                    media_grid_thw = media_inputs["video_grid_thw"]
...
class Qwen2_5OmniTemplate(Qwen2_5VLTemplate):
    ...
    def init_processor(self, processor) -> None:
        ...
        self.position_id_per_seconds = default["videos_kwargs"][
        ...
        if media_type == "image":
            ...
            if self.mode != "vllm":
                inputs.audios[index] = load_audio(
                    inputs.audios[index], self.sampling_rate
                )
            if self.version == "omni_v2_5":
                return ["<|audio_bos|><|AUDIO|><|audio_eos|>"]
            elif self.version == "omni_v3":
    ...
    def _get_new_tokens_use_audio_in_video(
        self,
        i,
        *,
        video_grid_thw,
        ...
        video_token_indices = (
            video_token_indices
            * video_second_per_grid[i]
            * self.position_id_per_seconds
        )
        if self.version == "omni_v2_5":
            tokens_per_chunk = int(
                self.position_id_per_seconds * self.seconds_per_chunk
            )
            video_chunk_indexes = self.processor.get_chunked_index(
                video_token_indices, tokens_per_chunk
            )
            audio_chunk_indexes = self.processor.get_chunked_index(
            ...
            for j in range(max(len(video_chunk_indexes), len(audio_chunk_indexes))):
                if j < len(video_chunk_indexes):
                    video_seq_length = (
                        video_chunk_indexes[j][1] - video_chunk_indexes[j][0]
                    )
                    res += video_token_id * video_seq_length
                if j < len(audio_chunk_indexes):
                    audio_seq_length = (
                        audio_chunk_indexes[j][1] - audio_chunk_indexes[j][0]
                    )
    ...
    def _post_encode(self, model, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        inputs_embeds = self._get_inputs_embeds_hf(
            inputs_embeds, inputs, model.thinker.visual, self.processor, thinker_config
        )
        if input_features is None:
            if is_deepspeed_enabled() and not is_deepspeed_zero3_enabled():
                ...
                input_features = input_ids.new_zeros(
                    [1, 128, 128], dtype=model.thinker.audio_tower.dtype
                )
            ...
            audio_mask = (
                (input_ids == thinker_config.audio_token_index)
                .unsqueeze(-1)
    ...
    def _data_collator_mm_data(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if input_features:
            if self.version == "omni_v3":
                max_length = max(
                    input_feature.shape[-1] for input_feature in input_features
                )
                for i, input_feature in enumerate(input_features):
                    mask = feature_attention_mask[i]
                    input_features[i] = F.pad(
                        input_feature, (0, max_length - input_feature.shape[-1])
                    )
                    feature_attention_mask[i] = F.pad(
                        mask, (0, max_length - mask.shape[-1])
                    )
            res["input_features"] = torch.concat(input_features)
            res["feature_attention_mask"] = torch.concat(feature_attention_mask)
        return res
...
register_template(
    QwenTemplateMeta(
        MLLMTemplateType.qwen3_omni,
        template_cls=Qwen3OmniTemplate,
        default_system=None,
        thinking_prefix="<think>\n",
    )
)
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        for i, idx in enumerate(idx_list):
            raw_pixel_values, image_placeholders = (
                self.model.visual_tokenizer.preprocess_image(
            ...
            if labels is not None:
                labels = (
                    labels[:idx] + [-100] * len(image_placeholders) + labels[idx + 1 :]
                )
...
class Ovis2_5Template(Template):
    ...
    def replace_tag(
        ...
        if media_type == "image":
            ...
            if self.mode == "vllm":
                ...
                inputs.images = load_video_ovis2_5(
                    inputs.videos[index], self.num_frames
                )
                return [[-200], "\n"]
    ...
    def _post_encode(self, model: nn.Module, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if pixel_values is not None:
            ...
            visual_embeds = model.vte(visual_tokens).to(
                dtype=inputs_embeds.dtype, device=inputs_embeds.device
            )
            ...
            inputs_embeds = inputs_embeds + visual_embeds.mean() * 0.0
